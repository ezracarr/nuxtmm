...
export default (algoliaConfig) => {
    ...
    return {
        get: async (eventId) => {
            try {
                return unWrap(await fetch(`https://${algoliaConfig.appId}-dsn.algolia.net/1/indexes/events/${eventId}`, {
                    headers,                    
                }))
            } catch(error){
        ...
        getByUserId: async (userId) => {
            try {
                return unWrap(await fetch(`https://${algoliaConfig.appId}-dsn.algolia.net/1/indexes/events/query`, {
                    headers,
                    method: 'POST',
                    body: JSON.stringify({
